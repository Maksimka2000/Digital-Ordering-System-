...
public class SetOfMenuItem : MenuItem
{
    ...
    // constructor
    [JsonConstructor]
    public SetOfMenuItem(Restaurant restaurant, string name, double price, string description, List<Food>? foods = null,
    ...
    public void UpdateDays(List<DayOfWeek>? days)
    {
        if (days != null && days.Count > 0)
        {
            if (days.Distinct().Count() != days.Count)
                throw new ArgumentException("Duplicate days are not permitted in SetOfMenuItem.");

            // Create a copy of the existing _days to avoid modifying it during iteration
            var currentDays = new List<DayOfWeek>(_days);
        ...
        {
            _days = Enum.GetValues<DayOfWeek>().ToList(); // Default to all days
        }
        else
        {
            throw new ArgumentException("You can't pass an empty list to UpdateDays().");
        }
    }
    ...
    public void AddFood(Food food)
    {
        ...
        if (food.Restaurant != Restaurant)
            throw new ArgumentException("Food you are trying to add belong to another restaurant.");
    ...
    {
        if (beverage is null)
            throw new ArgumentNullException($"{this.Name}. Beverages cannot be null in AddBeverage method.");
        // Validation: make sure menu item added to the setOfMenuItem belong to the same restaurant as SetOfMenuItem
        if (beverage.Restaurant != Restaurant)
            throw new ArgumentException("Beverage you are trying to add belong to another restaurant.");
    ...
    {
        if (beverage is null)
            throw new ArgumentNullException($"{this.Name}. Beverages cannot be null in RemoveBeverage method.");
        if (_beverages.Contains(beverage))
        {
            _beverages.Remove(beverage);
            Console.WriteLine(
                $"{this.Name} was modified by RemoveBeverage. So mind of the {beverage.Name}  doens't exist in SetOfMenuItem anymore..");
    ...
    public void UpdateBeverages(List<Beverage> beverages)
    {
        if (beverages.Count != 0)
        {
            if (_beverages.Count > 0)
                foreach (Beverage beverage in _beverages)
    ...
    private void ValidateTime(TimeSpan? startTime, TimeSpan? endTime)
    {
        if ((startTime.HasValue ^ endTime.HasValue))
            throw new ArgumentException("Start time and end time must be specified or nothing");
