...
public class RegisteredClient
{
    ...
    // constructor 
    [JsonConstructor]
    public RegisteredClient(string name, string password, string? email = null, string? phoneNumber = null,
        string? surname = null)
    {
        Id = ++IdCounter;
        Name = name;
    ...
    public void AddOnlineOrder(OnlineOrder onlineOrder)
    {
        if (onlineOrder == null)
            throw new ArgumentNullException(
                $"Parameter {nameof(onlineOrder)} cannot be null in AddOnlineOrder() RegisteredClient");
        if (onlineOrder.RegisteredClient == null)
            throw new ArgumentException(
                $"you can't add the online order directly to the user, it is done automatically after creation of the online order");
        if (onlineOrder.RegisteredClient != this)
            throw new AggregateException($"online order you are trying to add belong to different client");
    ...
    public void AddOrder(Order order)
    {
        ...
        if (order.RegisteredClient != null && order.RegisteredClient != this)
            throw new ArgumentException(
                $"Order {order.Id} has already client asigned to it: {order.RegisteredClient.Id}");
    ...
    private static void ValidateEmailAndPhoneNumberInput(string? email, string? phoneNumber)
    {
        if (email is null && phoneNumber is null)
            throw new NullReferenceException("Email and phone number cannot be null");
    }
    ...
    {
        if (newEmail == null && PhoneNumber == null)
        {
            throw new NullReferenceException("Email cannot be null if PhoneNumber is also null");
        }
    ...
    {
        if (newPhoneNumber == null && Email == null)
        {
            throw new NullReferenceException("PhoneNumber cannot be null if Email is also null");
        }
